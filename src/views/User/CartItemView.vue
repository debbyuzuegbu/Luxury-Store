  ...
  <section class="container">
    ...
    <div class="cart-item" v-if="loaded">
      ...
      <div class="product-details">
        ...
        <span>Home / {{ this.product && this.product.brand }}</span>
        <h4>{{ this.product && this.product.title }}</h4>
        <h4>{{ this.product && this.product.price }}</h4>
        <select v-model="size">
...
export default {
  ...
  async created() {
    let res = await axios.get(
      `https://dummyjson.com/products?id=${this.$route.params.id}`
    );
